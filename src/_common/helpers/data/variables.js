...
function getLocalStorageVariableInitValue(variable) {
    ...
    switch (variable.type) {
        case 'query':
        case 'string':
            value = localStorage.getItem(`variable-${variable.id}`) || variable.defaultValue;
            break;
        case 'number':
        case 'object':
        case 'array':
        case 'boolean':
            try {
                value = JSON.parse(localStorage.getItem(`variable-${variable.id}`)) || variable.defaultValue;
            } catch {
...
export function registerVariable(variableId) {
    const item = wwLib.$store.getters['data/getAllVariables'][variableId];
