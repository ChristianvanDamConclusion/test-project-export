...
export default class wwObjectHelper {
    ...
    static async create(type, content, _state, parentSectionId, keepChildren) {
    ...
    static async createFromTemplate(wwObjectTemplate, parentSectionId, keepChildren) {
    ...
    static async cloneElement(uid, parentSectionId, name) {
        const template = wwObjectHelper.convertToTemplate(uid);
        const newWwObjectId = await wwObjectHelper.createFromTemplate({ ...template, name }, parentSectionId);
    ...
    static getWwObject(uid) {
        if (uid.includes('_')) {
            const [parentId] = uid.split('_');
            return _.merge(
                _.cloneDeep(wwLib.$store.getters['websiteData/getWwObject'](parentId)),
                _.cloneDeep(wwLib.$store.getters['websiteData/getWwObject'](uid))
            );
        } else {
