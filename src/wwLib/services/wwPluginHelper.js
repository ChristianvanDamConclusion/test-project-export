...
export default {
    async registerPlugin(componentId, content, isDev = false) {
        const plugin =
            ...
            wwLib.$store.dispatch('websiteData/addDevPlugin', { name: componentId });

        if (!wwLib.wwPlugins) wwLib.wwPlugins = {};
        if (wwLib.wwPlugins[plugin.name] && !isDev) return;
